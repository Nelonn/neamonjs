...
exports.Command = class Command {
  constructor(name = '', type, required) {
    this.name = name;
    this.type = Object.values(exports.types).includes(type) ? type : null;
...
exports.parse = (commands = []) => {
  ...
  for (const cmd of commands) {
    ...
    if (!cmd.name.startsWith('-') && exports.warn) {
  ...
  for (let arg of args) {
    ...
    if (commandsObject[name]) {
      ...
      if (command.type) {
        if (command.type === exports.types.EqualSymbolValue) {
          parsed[name] = cmdArgs.join('=');
          if (commandsRequired.get(name)) commandsRequired.delete(name);
        } else if (command.type === exports.types.SpaceValue) {
